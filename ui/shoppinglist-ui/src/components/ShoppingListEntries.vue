...
<template>
    <div class="flex items-center bg-white rounded-md shadow mt-4 mx-2">
        <div class="pl-2">
        ...
        <input
            class="w-full rounded-md text-gray-700 leading-tight focus:outline-none py-3 px-2 border-0 focus:ring-0 ring-0"
            id="search" type="text" placeholder="Search or add" v-model="search" />
        <button v-if="search" class="mx-2 focus:shadow-outline focus:outline-none text-gray-500"
            @click="search = ''"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
    ...
    <div v-if="search" class="mt-2 flex justify-center items-center flex-wrap">
        ...
        <input class="ml-4 rounded-md bg-white shadow focus:outline-none border-0 focus:ring-0 ring-0 w-20" id="amount"
            type="text" placeholder="Amount" v-model="amount" />
        <button
            class="ml-4 shadow bg-emerald-500 hover:bg-emerald-700 focus:shadow-outline focus:outline-none text-white text-xs py-3 px-6 rounded"
            @click="addEntry(props.selectedlistId, search)">Add</button>
    </div>
    <div class="pt-2 text-sm flex flex-col flex-auto h-0 overflow-y-scroll px-2">
        <div v-for="entry in filteredResults" :key="entry.id"
            class="flex justify-start cursor-pointer text-gray-700 my-3 first:mt-0 last:mb-0 items-center">
            <label class="flex flex-grow justify-start items-start shrink">
                <input type="checkbox"
                    class="w-7 h-7 rounded-full transition-colors duration-400 border-emerald-500 border-2 focus:outline-0 focus:ring-0 focus:ring-offset-0"
                    :class="{ 'text-gray-400': !entry.buy, 'text-emerald-500': entry.buy }" :checked="!entry.buy"
                    @change="updateEntry(props.selectedlistId, entry)" />
                <div class="flex-grow text-xl ml-2" :class="{ 'text-gray-400 line-through': !entry.buy }">{{
                    entry.name
                }}</div>
            </label>
            <div class="font-normal mr-2" :class="{ 'text-gray-400': !entry.buy, 'text-gray-500': entry.buy }">{{
                entry.amount
            }}</div>
            <div class="dropdown">
                <button class="focus:outline-none h-7 flex items-center" type="button"
                    :class="{ 'text-gray-400': !entry.buy, 'text-gray-500': entry.buy }" aria-haspopup="true"
                    aria-expanded="true" aria-controls="headlessui-menu-items-117">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4" fill="none" viewBox="0 0 15 24"
